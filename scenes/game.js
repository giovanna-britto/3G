export class GameScene extends Phaser.Scene {
    ...
    update() {
        ...
        if (this.cursors.left.isDown) {
            ...
            // Troca de animação para "esquerda" se ainda não estiver ativa
            if (this.player.anims.currentAnim?.key !== 'esquerda') {
                this.player.anims.play('esquerda', true);
            }
        } else if (this.cursors.right.isDown) {
            this.player.setVelocityX(160);
            // Troca de animação para "direita" se ainda não estiver ativa
            if (this.player.anims.currentAnim?.key !== 'direita') {
                this.player.anims.play('direita', true);
            }
        } else {
            this.player.setVelocityX(0);
            // Troca de animação para "parada" se ainda não estiver ativa
